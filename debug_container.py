...
3. ??????:
   a) ???Dockerfile???:
      python debug_script.py dockerfile
   
   b) ???????????:
      docker build -f Dockerfile.fixed -t whisper-fixed .
   
   c) docker-compose.yml???:
      image: whisper-fixed

4. ??????:
   - ??????: python debug_script.py status
   - ????: python debug_script.py logs
   - ????????: python debug_script.py dirs
""")

def main():
    """?????"""
    print("Docker ???? ???????")
    print("realtime-whisper-subtitles-optimized ???????")
    print("=" * 50)
    ...
    if len(sys.argv) > 1:
        ...
        # ?????
        print("\n=== ????? ===")
        print("1. python debug_script.py fix - ???????")
        print("2. python debug_script.py dockerfile - ?????Dockerfile???")
        print("3. python debug_script.py help - ??????????")
